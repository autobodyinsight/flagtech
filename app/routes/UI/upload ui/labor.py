  ...
  <div class="modal-content">
    ...
    <div style="margin-top: 20px; display: flex; gap: 10px; justify-content: space-between;">
      ...
      <div style="display: flex; gap: 10px;">
        <button onclick="printLaborModal()" style='padding:10px 20px; font-size:14px; cursor:pointer; background-color:#505050; color:white; border:none; border-radius:3px;'>Print</button>
        <button onclick="saveLaborModal()" style='padding:10px 20px; font-size:14px; cursor:pointer; background-color:#505050; color:white; border:none; border-radius:3px;'>Save</button>
      </div>
    </div>
  </div>
</div>
...
def get_labor_modal_script(labor_items_json, total_labor, second_ro_line, vehicle_info_line):
  ...
  function addLaborAdditionalHours() {{
    ...
    itemDiv.innerHTML = '<input type="text" class="labor-addl-desc" placeholder="Enter description" />' +
                        '<input type="number" class="labor-addl-value" step="0.1" placeholder="0.0" onchange="updateLaborTotal()" /> hrs' +
  ...
  function toggleLaborDeduction(index) {{
  ...
  function openLaborModal() {{
    ...
    if (laborItems.length === 0) {{
      ...
      laborItems.forEach((item, index) => {{
        ...
        html += '<input type="checkbox" class="labor-item-checkbox" onchange="toggleLaborDeduction(' + index + ')" />';
        html += '<div style="flex: 1;"><strong>Line ' + item.line + '</strong> - ' + item.description + '</div>';
        html += '<div>' + formatLaborHours(item.value) + ' hrs</div>';
        html += '</div>';
      }});
    }}
  ...
  function printLaborModal() {{
    ...
    checkboxes.forEach((checkbox, index) => {{
      if (!checkbox.checked) {{
        ...
        printContent += ' <div style="display: inline; float: right;">' + formatLaborHours(laborItems[index].value) + ' hrs</div>';
        printContent += '</div>';
    ...
    additionalDescs.forEach((descInput, index) => {{
      ...
      if (desc || val) {{
        ...
        printContent += ' <div style="display: inline; float: right;">' + formatLaborHours(val) + ' hrs</div>';
        printContent += '</div>';
        totalLabor += val;
      }}
    }});
  ...
  // ------------------------------------------------------
  // UPDATED SAVE FUNCTION — POSTS TO BACKEND (NO DOWNLOAD)
  // ------------------------------------------------------
  function saveLaborModal() {{
    ...
    additionalDescs.forEach((descInput, index) => {{
      ...
      const val = parseFloat(additionalValues[index].value) || 0;
      if (desc || val) {{
        additionalItems.push({{ description: desc, value: val }});
      }}
    }});
    ...
    .then(res => res.json())
    .then(response => {{
      alert("Labor assignment saved.");
      closeLaborModal();
    }})
    .catch(err => {{
      console.error(err);
      alert("Error saving labor assignment.");
    }});
  }}
"""